...
export default function Leaderboard() {
  ...
  useEffect(() => {
    // 1. Admin tournaments aur registered users ka data fetch karein
    const allTournaments = JSON.parse(localStorage.getItem("adminTournaments")) || [];
    const earningsMap = {};

    // 2. Tournaments se registered users aur unki winnings/earnings calculate karein
    allTournaments.forEach((t) => {
      if (t.registeredUsers && Array.isArray(t.registeredUsers)) {
        t.registeredUsers.forEach((user) => {
          ...
          // Prize pool ya entry fee ke hisaab se earnings add karein
          earningsMap[user] += parseFloat(t.prize) || 500;
        });
      }
    });

    // 3. Agar koi real user registered nahi hai, tabhi default players dikhayein taaki page khali na lage
    ...
    // 4. Tab filters ke hisaab se earnings adjust karein
    if (activeTab === "Daily") {
            ...
            <div key={index} style={{ 
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: "9px", color: "#9ca3af" }}>Live User</span>
                </div>
              </div>
